...
for i in range(user_items):
    ...
    final_price = int(flair_prices[index_flair]) + int(main_prices[index_main])
...
meal_price = sum(int(flair_prices[mains.index(main)]) + int(main_prices[mains.index(main)]) for main in random.choices(mains, k=user_items))
