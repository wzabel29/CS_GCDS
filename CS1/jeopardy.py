...
4) pop culture 
5) movies""")
    category = input("Choose the number of the category you would like to play: ")
    while category not in ["1", "2", "3", "4", "5"]:
        category = input("please enter the number of the category you would like: ")
    mapping = {"1": "science", "2": "history", "3": "python", "4": "popculture", "5": "movies"}
    return mapping[category]

def money_chooser():
    point_scores = ["200", "400", "600", "800", "1000"]
    money = input("please enter a cash value of 200, 400, 600, 800, or 1000 as a number: ")
    while money not in point_scores:
        money = input("please enter a cash value of 200, 400, 600, 800, or 1000 as a number: ")
    return money

def score_checker(score, bot_score):
    print(f"\nyour score is ${score} and enemy score is ${bot_score}.")
    if score > bot_score:
        print("you won!!")
...
def main():
    ...
    questions_dict = {
        "science": science_questions, "history": history_questions, 
        "python": python_questions, "popculture": popculture_questions, "movies": movies_questions
    }
    answers_dict = {
        "science": science_answers, "history": history_answers, 
        "python": python_answers, "popculture": popculture_answers, "movies": movies_answers
    }

    for round in range(num):
        category = category_chooser()
        money = money_chooser()
        
        money_index = point_scores.index(money)
        
        question = questions_dict[category][money_index]
        print(f"\nQuestion: {question}")
        
        user_answer = input("answer(enter proper nouns with uppercase): ")
        
        if user_answer == answers_dict[category][money_index]:
            print("correct!")
            score += int(money)
        else:
            print(f"incorrect. The right answer was {answers_dict[category][money_index]}.")
            bot_score += int(money)
