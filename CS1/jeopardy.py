...
5) movies""")
    category = input("Choose a category: ")
    while True:
        if category != "1" or "2" or "3" or "4" or "5":
...
def money_chooser():
    point_scores = ["200", "400", "600", "800", "1000"]
    money = input("please enter a cash value of 200, 400, 800, or 1000 as a number:")
    while True:
        if money not in point_scores:
            money = input("please enter a cash value of 200, 400, 800, or 1000 as a number:")
        else: 
            return money
...
def main(category, money, num):
    ...
    for round in range(num):
        ...
        money_chooser()
        money_index = point_scores.index(money)
        question = {category}_questions[money_index]
        print(question)
        user_answer = input("answer(enter proper nouns with uppercase):")
        if user_answer == {category}_answers[money_index]:
            print("correct")
            score += int(money)
        else:
            print("incorect")
